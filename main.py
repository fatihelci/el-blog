    ...
    def decorated_function(*args, **kwargs):
        if current_user.is_authenticated:
            if current_user.id != 1:
                return redirect(url_for(abort(403)))
        elif AnonymousUserMixin:
            return redirect(url_for(abort(403)))
...
class BlogPost(db.Model, Base):
    ...
    texts = relationship("Comment", back_populates="parent_post")
...
class Comment(db.Model, Base):
    ...
    parent_post = relationship("BlogPost", back_populates="texts")
...
def get_all_posts():
    ...
    return render_template("index.html", all_posts=all_posts, admin=admin)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash(message="No such user", category="danger")
        elif not check_password_hash(user.password, form.password.data):
            flash(message="Wrong password", category="danger")
        else:
            login_user(user)
